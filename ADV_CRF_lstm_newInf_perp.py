...
class GAN_CRF_model(object):
		...
		"""
                ...
                for idx, p in enumerate(a_params):
                        p.set_value(PARA[idx])		
	        """
                l_in_word_a0 = lasagne.layers.InputLayer((None, None))
                l_emb_word_a0 = lasagne_embedding_layer_2(l_in_word_a0, embsize, l_emb_word.W)
                #l_emb_word_a = lasagne.layers.EmbeddingLayer(l_in_word_a,  input_size=We_initial.shape[0] , output_size = embsize, W =We)
                if params.dropout:
                           l_emb_word_a0 = lasagne.layers.DropoutLayer(l_emb_word_a0, p=0.5)

                l_lstm_wordf_a0 = lasagne.layers.LSTMLayer(l_emb_word_a0, hidden, mask_input=l_mask_word_a)
                l_lstm_wordb_a0 = lasagne.layers.LSTMLayer(l_emb_word_a0, hidden, mask_input=l_mask_word_a, backwards = True)
                l_reshapef_a0 = lasagne.layers.ReshapeLayer(l_lstm_wordf_a0 ,(-1, hidden))
                l_reshapeb_a0 = lasagne.layers.ReshapeLayer(l_lstm_wordb_a0 ,(-1,hidden))
                concat2_a0 = lasagne.layers.ConcatLayer([l_reshapef_a0, l_reshapeb_a0])

                l_local_a_inf = lasagne.layers.DenseLayer(concat2_a0, num_units= 25, nonlinearity=lasagne.nonlinearities.softmax)
                ...
                predy0_inf = lasagne.layers.get_output(l_local_a_inf, {l_in_word_a0:g, l_mask_word_a:gmask})
                ...
                if (params.margin_type==1):
                        ...
                        hinge_cost0 = 0.1*delta0 + neg_cost  - pos_cost
                ...
                if (params.regu_type ==0):
                      g_cost = T.mean(-hinge_cost_inf) +  T.mean(ce_hinge_inf)
                else:
                      g_cost = T.mean(-hinge_cost_inf)

               
           
                msvm = T.sum(T.gt(hinge_cost0, 0.0))
                perp = T.sum(T.gt(hinge_cost_inf, 0.0))       
  
                d_cost = T.mean(T.maximum(hinge_cost_inf, 0.0))
                ...
                a_params = lasagne.layers.get_all_params(l_local_a_inf, trainable=True)
                ...
                self.train_g = theano.function([g, gmask, y, y_in, length], [g_cost, d_cost, pos_cost, neg_cost, msvm, perp], updates=updates_g, on_unused_input='ignore')	
                ...
                self.train_d = theano.function([g, gmask, y,  y_in, length], [d_cost, g_cost, pos_cost, neg_cost, msvm, perp], updates=updates_d, on_unused_input='ignore')
	        ...
	        """build the function for the test time inference"""
                ...
                self.test_time = theano.function([g, gmask, y, length] , acc_inf)
                        ...
                        for eidx in xrange(500):
                                ...
                                for _, train_index in kf:
                                        ...
                                        for ii in range(params.Lambda):
                                             #if eidx < 30:                       					
                                             g_cost, hingeloss_g, pos_cost, neg_cost, g_msvm, g_perp = self.train_g(x0, x0mask, y0,  y0_in, maxlen)
                                             #else:
                                             #        g_cost, hingeloss_g, pos_cost, neg_cost, delta, pred0 = self.train_g_later(x0, x0mask, y0,  y0_in, maxlen)

                                        d_cost, hingeloss_d, pos_cost, neg_cost, msvm, perp = self.train_d(x0, x0mask, y0,  y0_in, maxlen)
                                        aa += msvm
                                        bb += perp                                
                                ...
                                print 'after d_step:  inf positive', g_aa, ' cost-augmented positive', g_bb
                                print 'after g_step: inf positive', aa, ' cost-augmented positive', bb
                                end_time1 = time.time()

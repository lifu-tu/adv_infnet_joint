		...
		"""
	        ...
	        """
                ...
                if (params.regu_type ==0):
                        g_cost = T.mean(-hinge_cost0) + 10*T.mean(-hinge_cost_inf) + T.mean(ce_hinge) + T.mean(ce_hinge_inf)
                else:
                        g_cost = T.mean(-hinge_cost0) + 10*T.mean(-hinge_cost_inf)
                #g_cost_later = T.mean(-hinge_cost0)
                 
  
                d_cost = T.mean(T.maximum(hinge_cost0, 0.0)) + params.Lambda*T.mean(T.maximum(hinge_cost_inf, 0.0))

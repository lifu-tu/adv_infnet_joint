...
class GAN_CRF_model(object):
		...
		"""
                ...
                l_in_label = lasagne.layers.InputLayer((None, 50))
                #l_in_label2 = lasagne.layers.InputLayer((None, None, 25))
                #concat2_l = lasagne.layers.ConcatLayer([l_in_label1, l_in_label2], axis=2)
                #l_lstm_wordf_l = lasagne.layers.LSTMLayer(l_in_label, hidden, mask_input=l_mask_word_a)
                #l_lstm_wordb_l = lasagne.layers.LSTMLayer(l_in_label, hidden, mask_input=l_mask_word_a, backwards = True)
                #concat2_l = lasagne.layers.ConcatLayer([l_lstm_wordf_l, l_lstm_wordb_l], axis=2)
                #concat2_l = lasagne.layers.ReshapeLayer(concat2_l ,(-1,2*hidden))
                l_local_a = lasagne.layers.DenseLayer(l_in_label, num_units= 25, nonlinearity=lasagne.nonlinearities.softmax)
	        ...
	        """
                ...
                predy0 = lasagne.layers.get_output(l_local_a, {l_in_label:new_inf_input})
                ...
                d_cost1 = T.mean(T.maximum(hinge_cost_inf, 0.0))                
                ...
                grad1 = T.grad(d_cost1, d_params)
                norm1 = T.sqrt(sum(T.sum(tensor**2) for tensor in grad1))

                grad2 = T.grad(T.mean(neg_inf_cost), a_params, disconnected_inputs='ignore')
                norm2 = T.sqrt(sum(T.sum(tensor**2) for tensor in grad2))



                self.train_g = theano.function([g, gmask, y, y_in, length], [g_cost, norm1, pos_cost, neg_cost, delta0, check], updates=updates_g, on_unused_input='ignore')	
                ...
                self.train_d = theano.function([g, gmask, y,  y_in, length], [d_cost, norm1, norm2, neg_cost, delta0, check], updates=updates_d, on_unused_input='ignore')
                ...
                try:
                        for eidx in xrange(500):
                                ...
                                for _, train_index in kf:
                                        ...
                                        g_cost, norm1, pos_cost, neg_cost, delta, pred0 = self.train_g(x0, x0mask, y0,  y0_in, maxlen)

                                        for ii in range(params.Lambda-1):
                                             #if eidx < 30:                       					
                                             g_cost, _, pos_cost, neg_cost, delta, pred0 = self.train_g(x0, x0mask, y0,  y0_in, maxlen)
                                             #else:
                                             #        g_cost, hingeloss_g, pos_cost, neg_cost, delta, pred0 = self.train_g_later(x0, x0mask, y0,  y0_in, maxlen)

                                        d_cost, _, norm2, neg_cost, delta, pred0 = self.train_d(x0, x0mask, y0,  y0_in, maxlen)
                                        """
                                        aa += hingeloss_g
                                        bb += hingeloss_d                                
					"""
                                ...
                                end_time1 = time.time()
                                self.textfile.write("energy gradient norm:%f  inf: %f    \n" %(  aa, bb)  )
                                self.textfile.flush()

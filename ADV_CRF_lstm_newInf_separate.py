...
class GAN_CRF_model(object):
		...
		"""
	        ...
	        """
                ...
                delta1 = T.sum(abs((y_in - predy_inf)), axis=2)*gmask
                delta1 = T.sum(delta1, axis=1)
                ...
                if (params.margin_type==1):
                        ...
                        hinge_cost_inf = 1 + neg_inf_cost  - pos_cost
                        ...
                        hinge_cost_inf = delta1 + neg_inf_cost  - pos_cost
                elif(params.margin_type==3):
                        hinge_cost0 =  delta0*(1.0 + neg_cost  - pos_cost)
                        hinge_cost_inf = delta1*(1.0 + neg_inf_cost  - pos_cost)


                #delta1 = T.sum(abs((predy_inf - predy)), axis=2)*gmask
                #delta1 = T.sum(delta1, axis=1)
 
                #g_cost = T.mean(neg_cost - delta1) + params.Lambda*T.mean(-hinge_cost_inf)
                if (params.regu_type ==0):
                      g_cost = T.mean(-neg_cost - delta0) + T.mean(-neg_inf_cost) +  T.mean(ce_hinge_inf) + T.mean(ce_hinge)
                else:
                      g_cost = T.mean(-neg_cost - delta0) + T.mean(-neg_inf_cost)
                #g_cost_later = T.mean(-hinge_cost0)
                 
  
                d_cost = T.mean(T.maximum(hinge_cost0, 0.0)) + T.mean(T.maximum(hinge_cost_inf, 0.0))
	        ...
	        """build the function for the test time inference"""
                ...
                self.test_time = theano.function([g, gmask, y, length] , [acc_inf, acc_cost])                
        ...
        def train(self, trainX, trainY, devX, devY, testX, testY, params):	
                ...
                devacc, _  = self.test_time(devx0, devx0mask, devy0, devmaxlen)
                testacc, _  = self.test_time(testx0, testx0mask, testy0, testmaxlen)
                ...
                try:
                        for eidx in xrange(500):
                                ...
                                devacc, devacc_cost  = self.test_time(devx0, devx0mask, devy0, devmaxlen)
                                end_time2 = time.time()
                                print 'dev acc', devacc
                                testacc, testacc_cost = self.test_time(testx0, testx0mask, testy0, testmaxlen)
